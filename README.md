...
3. Create Momentum Transformer input features with: `python -m src.examples.create_features_pinnacle`. In this example we use the 100 futures tickers which have i) the longest history ii) more than 90% of trading days have data iii) data up until at least Dec 2021.
4. Optionally, run the changepoint detection module: `python -m src.examples.concurent_cpd_pinnacle <<CPD_WINDOW_LENGTH>>`, for example `python -m src.examples.concurent_cpd_pinnacle 21` and `python -m examples.concurent_cpd_pinnacle 126`
5. Create Momentum Transformer input features, including CPD module features with: `python -m src.examples.create_features_pinnacle 21` after the changepoint detection module has completed.
6. To create a features file with multiple changepoint detection lookback windows: `python -m src.examples.create_features_pinnacle 126 21` after the 126 day LBW changepoint detection module has completed and a features file for the 21 day LBW exists.
7. Run one of the Momentum Transformer or Slow Momentum with Fast Reversion experiments with `python -m src.examples.run_dmn_experiment <<EXPERIMENT_NAME>>`
