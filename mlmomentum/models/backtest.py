...
# Legacy code removed - IntermediateStrategy class now in strategies.momentum.intermediate


def run_classical_methods(
        features_file_path,
        train_intervals,
        reference_experiment=None,  # Changed to have default None
    ...
    Args:
        ...
        train_intervals ([type]): list of train/test intervals
        reference_experiment (str, optional): other experiment, testing against. If None, returns will be extracted directly from features file. Defaults to None.
    ...
    """
    # TODO: This function needs to be properly refactored to use the new IntermediateStrategy class
    # from strategies.momentum.intermediate import IntermediateStrategy
    ...
    for train_interval in train_intervals:
        ...
        # Legacy position calculation - to be replaced with IntermediateStrategy 
        returns_data["position"] = (
            0 * np.sign(returns_data["norm_monthly_return"]) + 
            (1 - 0) * np.sign(returns_data["norm_annual_return"])
        )
