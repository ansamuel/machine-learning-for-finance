...
class DeterministicLSTMEncoder(nn.Module):
    ...
    def forward(self, context_x, context_y, target_x=None):
        ...
        if self.use_x_attn:
            ...
            hidden_v = torch.zeros(batch_sz, context_len, num_contexts, self.hidden_dim).to(device)
            ...
            hidden_k = torch.zeros(batch_sz, context_len, num_contexts, self.hidden_dim).to(device)
            for i in range(num_contexts):
