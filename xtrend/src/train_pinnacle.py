...
RUN_NAME = "test_lre-3-4lat-5min-w-pred"
...
train_data_prepped_all_segments = train_data_prepped_all_segments[train_data_prepped_all_segments["seq_len"] >= MIN_SEQ_LEN].copy()

test_data_prepped_all_segments = pd.read_pickle("data/prepped_test.pkl")
test_data_prepped_all_segments = test_data_prepped_all_segments[test_data_prepped_all_segments["seq_len"] >= MIN_SEQ_LEN].copy()
...
for it in range(ITERATIONS):
    ...
    for start in range(0, len(shuffled), BATCH_SIZE):
        ...
        captured_return.append(
            (torch.mean(torch.sign(pred_y[:, :, :]) * y_target[:, -1:, :]))
            .detach()
            .item()
        )
    ...
    for key, test in test_data_prepped_all_segments.iterrows():
        ...
        pred_y_test, sigma, test_loss, _ = model.forward(
        ...
        test_results.loc[key, "captured_return"] = (
            (torch.mean(torch.sign(pred_y_test[:, :, :]) * y_target[:, -1:, :]))
            .detach()
            .item()
        )
        # break
