...
data = features[
    features.date.dt.year >= START_TARGET_YEAR
].copy()  # make sure there are at least some contexts


all_dates = features[["date", "ticker"]].copy()
all_dates["cc"] = features.groupby(["ticker"]).cumcount()
all_dates = all_dates[all_dates["cc"] >= SEQ_LEN - 1]
all_dates = all_dates.drop(columns="cc")
...
def prep_epoch(data, start_year, end_year):
    data = data[
        (data.date.dt.year >= start_year) & (data.date.dt.year < end_year)
    ].copy()

    data["cc"] = data.groupby("ticker").cumcount()
    data["seq_num"] = (data.cc - random.randint(0, SEQ_LEN)) // SEQ_LEN
    data["seq_count"] = data.groupby(["ticker", "seq_num"])["date"].transform(
        "count"
    )  # .reset_index()
    data = data[data["seq_count"] == SEQ_LEN]
    data.index = data.groupby(["ticker", "seq_num"]).ngroup()

    N = len(data) // SEQ_LEN
    order = list(range(N))
    np.random.shuffle(order)
    targets = data.loc[order[: len(order) // BATCH_SIZE * BATCH_SIZE]].drop(
        columns=["cc", "seq_num", "seq_count"]
    )

    targets_x = torch.Tensor(targets[FEATURES].values)
    targets_x = targets_x.reshape(
        targets_x.shape[0] // BATCH_SIZE // SEQ_LEN, BATCH_SIZE, SEQ_LEN, len(FEATURES)
    )

    targets_y = torch.Tensor(targets["target_returns"].values)
    targets_y = targets_y.reshape(
        targets_y.shape[0] // BATCH_SIZE // SEQ_LEN, BATCH_SIZE, SEQ_LEN, 1
    )

    all_indices = (
        targets["date"]
        .iloc[range(SEQ_LEN - 1, len(targets), SEQ_LEN)]
        .map(
            lambda d: np.random.choice(dict_context_dates[d], size=NUM_CONTEXT).tolist()
        )
        .tolist()
    )

    all_indices_concat = list(itertools.chain.from_iterable(all_indices))
    contexts = features.iloc[
        list(
            itertools.chain.from_iterable(
                [list(range(l - SEQ_LEN + 1, l + 1)) for l in all_indices_concat]
            )
        )
    ]
    contexts_x = torch.Tensor(contexts[FEATURES].values)
    contexts_x = contexts_x.reshape(
        len(contexts_x) // BATCH_SIZE // NUM_CONTEXT // SEQ_LEN, BATCH_SIZE, NUM_CONTEXT, SEQ_LEN, len(FEATURES)
    )
    contexts_x = contexts_x.swapaxes(2, 3).swapaxes(3, 4)

    contexts_y = torch.Tensor(contexts["target_returns"].values)
    contexts_y = (
        contexts_y.reshape(
            len(contexts_y) // BATCH_SIZE // NUM_CONTEXT // SEQ_LEN, BATCH_SIZE, NUM_CONTEXT, SEQ_LEN
        )
        .swapaxes(2, 3)
        .unsqueeze(3)
    )

    return contexts_x, contexts_y, targets_x, targets_y


# if TESTING:
#     all_segments = pd.read_pickle("data/features_cpd_63lbw.pkl")
# else:
#     all_segments = pd.read_pickle("data/features_cpd_63lbw.pkl")
...
for it in range(ITERATIONS):
    ...
    contexts_x, contexts_y, targets_x, targets_y = prep_epoch(
        data, START_TARGET_YEAR, VALID_START_YEAR
    )
    ...
    for i in range(targets_x.shape[0]):
        ...
        x_context = contexts_x[i].to(device)
        y_context = contexts_y[i].to(device)
        x_target = targets_x[i].to(device)
        y_target = targets_y[i].to(device)

        train_loss = model.forward(x_context, y_context, x_target, y_target,)
        optimizer.zero_grad()
        train_loss.backward()
        optimizer.step()
    ...
    for i in range(val_targets_x.shape[0]):

        x_context = val_contexts_x[i].to(device)
        y_context = val_contexts_y[i].to(device)
        x_target = val_targets_x[i].to(device)
        y_target = val_targets_y[i].to(device)
